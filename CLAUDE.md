...
- **`__ExecTarget` (mkexec.pas) recursion→iteration** — the target-execution engine no longer recurses natively; it walks an explicit heap-allocated frame stack (`New`/`Dispose`, chained via `pPrev`) instead, since each native call frame carried a `TIdentifierPair` (81+256 bytes) plus more short strings — on MSX/TP3.3f's few-KB stack, 3-4 levels of ordinary prerequisite chaining could exhaust it. Verified behavior-identical to the old recursive version via byte-for-byte output diff across all `tools/hmake/samples` makefiles (FPC build); not yet re-verified on real MSX-DOS2 hardware
...
- **Built-in `$(MACHINE)`/`$(ARCH)` variables** — seeded as default entries in `handle.variableList` by `MkInit` (`mkutils.pas`), so a makefile assignment to either name overrides them (same last-write-wins lookup as any other variable). Both are platform-specific (`ctMkMachine`/`ctMkArch` in each `mkoscall.pas`): on MSX-DOS, `MACHINE='MSX'` and `ARCH='Z80'` (hardcoded — TP3.3f only ever targets the one CPU family, and no R800-specific `.asm` modules exist yet in `src/asm/` for a makefile to select between; placeholder for when that changes). On an FPC host, `MACHINE` is `LINUX`/`MACOSX`/`WINDOWS` via `{$IFDEF DARWIN}`/`{$IFDEF WINDOWS}`, and `ARCH` reflects the **actual host CPU hmake was compiled for** (`x86_64`, `aarch64`, ...) via FPC's `%FPCTARGETCPU%` compile-time macro — hmake is a general-purpose build tool, not exclusively a Hinotori/MSX build driver, so on that host it must report the real host architecture for makefiles building other projects there, not MSX's Z80
